...
## ⑥-1 샷 타입 분류 (CLOSEUP / FULLBODY) [v1.1 신설]

### 원칙: 대본의 모든 컷은 샷 타입을 명시적으로 분류해야 함

|샷 타입|용도|이미지 생성 방식|프롬프트 접두어|
|-|-|-|-|
|CLOSEUP|표정·감정·조명 집중|레퍼런스 이미지(yeori-closeup.jpg) 직접 활용|`CLOSEUP SHOT — ` 필수|
|FULLBODY|씬·의상·체형 집중|Flow 생성 (레퍼런스 의존도 낮음)|`FULLBODY SHOT — ` 필수|

### 대본 생성 시 규칙:
```
□ 각 컷마다 "샷 타입:" 필드에 CLOSEUP 또는 FULLBODY 명시 (필수, 누락 시 재생성)
□ 이미지 프롬프트 본문은 반드시 "CLOSEUP SHOT —" 또는 "FULLBODY SHOT —" 로 시작
□ CLOSEUP 컷: 시그니처 디테일(피부 텍스처) 강조, 표정 변화 위주로 서술
□ FULLBODY 컷: 의상·소품·자세·동선 위주로 서술, K-model proportions 강조
```

이 분류는 npm run flow(이미지 자동화) 단계에서 처리 방식을 다르게 적용하는 데 사용된다.
