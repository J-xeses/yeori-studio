...
### ⑬-3 대사·나레이션 텍스트의 문장부호 = TTS 프로소디 표현 [v1.4.4 신설 — 2026-09-11]

**발견 (사용자, 2026-09-11)**: TTS 탭 속도/안정성/유사도 슬라이더는 **컷(트랙) 전체**에 걸리는 값이라
컷 단위 톤만 바뀐다. 반면 `DL:`/`NR:` **텍스트 안의 문장부호는 그 지점에서만** 억양·쉬는 간격을 바꾼다 —
정제(`cleanForTTS`)·읽기교정(`applyReadings`) 어디서도 지워지지 않고 ElevenLabs 로 그대로 들어간다
(검증: `,` `~` `~~` `…` 전부 원문 유지 확인).
...
| 부호 | 효과 | 예시 |
|---|---|---|
| `,` 쉼표 | 짧게 끊어 쉼(숨 고르는 지점) | `저는요, 음, 그러니까~` |
| `~` 물결표 | 앞 음절을 늘여 발음 (느긋·애교·강조). `~~`면 더 길게 | `아~~ 대박`, `그니까~ 진짜` |
| `…` 말줄임표 | 쉼표보다 긴 정지 — 망설임·여운 | `음… 그게` |
| `!` `?` (중복 가능) | 억양을 올림, 개수로 강도 조절 | `대박!!`, `진짜?!` |

- 대본 작성(Codi_Gen/대본생성 탭) 시 `DL:`/`NR:` 필드에 이 부호를 **의도적으로** 배치해 억양을 설계한다.
  컷당 속도·안정성 슬라이더(§TTS 탭)로 큰 톤을, 문장부호로 문장 내 세부 리듬을 잡는 2단 구조.
- UI 참고: TTS 탭 "🎚 목소리 제어 가이드"에 동일 표가 내장돼 있음 (실시간 참조용).
- 파서 영향 없음(신규 필드 아님, 기존 DL/NR 텍스트 그대로 활용) — v3 파서 변경 불필요.
...
v1.4.4 (2026-09-11) — ⑬-3 대사·나레이션 문장부호 = TTS 프로소디 표현 신설(사용자 발견).
                     쉼표(짧은 끊음)·물결표(음절 늘임)·말줄임표(긴 정지)·느낌표/물음표(억양↑)가
                     정제·읽기교정 어디서도 지워지지 않고 그대로 TTS 로 들어감(검증 완료).
                     대본 작성 시 의도적 배치 규칙 명문화. TTS 탭 "목소리 제어 가이드"에도 동일 표 추가.
```
