...
## 🔌 yeori-studio MCP 도구가 실패할 때 (필독)

`studio_get_status` 등 yeori-studio MCP 도구가 오류를 내면 — **"연결 안 됨"으로 단정하고
"지금까지 작업 기준으로 정리할게요"라고 넘어가지 말 것.** Cloudflare Quick Tunnel 이
가끔 끊기지만 **워치독이 90초 간격으로 자동 재연결**한다(2026-09-08 추가).

1. **30~60초 뒤 같은 도구를 다시 호출.** 대개 복구돼 있음.
2. 2~3회 재시도해도 계속 실패하면 → 사용자에게 "PC에서 `YeoriMcpTunnel` 작업 상태 확인,
   또는 `start_yeori.bat` 실행" 요청.
3. 그 전까지는 **스튜디오 상태를 모른다고 답할 것.** 기억·추측으로 파이프라인 상태를
   보고하지 말 것 (컷 승인 현황 등은 실시간이라 기억이 틀림).

터널 URL 은 `.tunnel-state.json` / Vercel Edge Config `yeori-mcp-bridge` 의 `mcpBridgeUrl`
에 있고, `api/mcp.js` 가 요청 시점에 읽어 재배포 없이 반영된다.
...
---
### 2026-09-07 (MCP 자동 기록)

## 2026-09-07 — 스크립트 컷 코드 등록 규칙 확정

### v3 파서 컷 번호 체계 (LF_T01 검증 완료)
- 파서는 `[CUT N]` 숫자 형식만 인식 — 알파벳 포함 시 파싱 실패
- 확정 번호 체계:
  - 1~12 → YEORI 메인 컷
  - 91, 92 → 시그니처 서브컷 (9A→91, 9B→92)
  - 101, 102 → GRAPHIC 컷 (인트로/엔드카드)
  - 201~206 → BROLL/MV클립
- 각 컷 `CT:` 필드 필수 (YEORI / BROLL / GRAPHIC)
- 업로드 전 체크: 알파벳 헤더 없는지, CT 필드 전체 존재하는지 확인

### LF_T01 대본 등록 완료
- v7 스크립트 기준 22컷 전체 대본탭 등록 완료
- 자막 기능 8종 메이킹탭 추가 완료 (커밋 57e3d85)
...
## Field Gate (yeori-genline) 대규모 실사용 버그픽스 + 세그 빌더 신설

어제 CUT2 실사용 중 발견된 3단 구조적 문제를 전부 추적·수정하고 실사용 검증 완료.

- **원인 1**: `diagnose()`가 매턴 단발 API 호출이라 이전 턴 합의(분할·초 배분)를 기억 못함 → 대화 이력을 실제 멀티턴으로 API에 전달하도록 수정.
- **원인 2**: "대본에 반영 → 적용"해도 스튜디오 파서(`ScriptGenTab.jsx`)가 `SEG|DL|NR` 3개 필드만 인식해서 genline이 쓴 `SC:/SH:/DU:/CLIP A-B` 형식은 조용히 무시됨 → 전체 v3 필드 어휘(`V3_MAIN_FIELD_RE`) + 검증된 매퍼(`v3RevisionPatch`) 재사용으로 교체.
- **원인 3**: genline 시스템 프롬프트가 유효 필드명을 제한 안 해 "CLIP A/B" 같은 비표준 필드를 지어냄 → SEG 필드(8/10초 조합) 사용 규칙 명시.
- 부가 수정: 대화 초기화 시 서버 백업(전체 스레드 소실 방지), Veo 2클립 우선 진단(8초 단위로만 셈해 3클립부터 제안하던 편향 수정), 발화 시점 초단위 정밀화.
- **세그 분할 탭 전면 개편**: 하드코딩 후보 나열 → 2컷/3컷 유형 선택 + 슬롯별 생성초(8/10) 선택 + 원문 대사 드래그선택·직접편집 배정 인터랙티브 빌더.
- LF_T01 CUT2 실데이터(scene/duration/segments/videoPrompt)를 어제 합의안(SEG 10+8, 18초)으로 동기화.
...
---
### 2026-09-20 (MCP 자동 기록)
## 영상 생성 반자동화(Flow 무료 경로) 구축 + 영상탭·크레딧·대시보드 정비 (2026-09-20~21 세션)

※ 9/13~9/19 작업은 이 절에 포함하지 않음(STATUS 공백 구간 — 필요 시 메모리/커밋 기준 별도 보완).

### 1. Flow 반자동 제출 시스템 (신규, 전 구간 실측)
- `scripts/lib/flowDriver.js`(화면 조작) · `scripts/flow-submit.js`(한 클립 제출 잡) · 서버 `POST /api/flow/submit`·`GET /api/flow/job/:id`·`GET /api/flow/ready`·`POST /api/flow/cancel` · 영상탭 빈 슬롯/단일 클립 컷 "⚡ Flow 제출"·"✋ 취소" 버튼.
- 흐름: VP에서 클립별 프롬프트 추출 → 시작 프레임(클립1=G2 승인 이미지, 클립2+=앞 클립 마지막 프레임, 업로드명에 sha1 지문) → 프롬프트 한 줄 입력(개행=Enter=제출이라 공백 치환) → **검증 관문**(모델·비율·길이·x1·예상 크레딧 상한·시작프레임·프롬프트 원문 일치, 하나라도 어긋나면 전송 안 함) → 전송 → 완료 감지 → 뷰어 다운로드("720p 원본 크기"만, 1080p/4K는 업그레이드 필요) → `raw/cut_NN_clip_K.mp4` 저장 → 슬롯 자동 배정 + Flow 크레딧 자동 차감.
- 실측: 컷14 클립2 실제 생성(Omni 1.1 Flash 8초 12크레딧, 152초), 자동 다운로드가 사용자 수동 다운로드와 md5 동일. 드라이런으로 컷22(8초 12)·컷12(10초 15)·취소 검증. 첫 시도는 사유 없이 "생성할 수 없습니다"(미청구) 후 재시도 성공 → 간헐 실패 가능.
- Flow 화면 구조: 서브모드 프레임(시작⇄종료)/소재, 모델 Omni 1.1 Flash·Veo 3.1 Lite/Fast/Quality, 길이(4/6/8/10초)·해상도는 Omni에서만 선택(Veo는 8초 고정으로 보임), Veo Quality 8초=100크레딧, "생성 시 N 크레딧" 줄은 팝업 열려있을 때만 표시.
- 🚨 **사고(9/20, 복구 완료)**: 입력칸 포커스 확인 없이 Ctrl+A→Backspace를 눌러 Flow 프로젝트 66개 항목이 휴지통으로 이동 → 토스트 "실행취소"로 즉시 복구. 이후 모든 키 입력은 입력칸 포커스 검증 후에만. 규칙: Flow에서 Ctrl+A/Backspace/Delete 계열은 포커스 검증 없이 절대 금지.
- 이번 에피소드(LF_T01)는 클립 31개가 사용자 수동 업로드로 이미 모두 채워져 있어 **UI 버튼 통합 시험은 미실시** → 다음 에피소드 첫 빈 슬롯에서 확인 필요.

### 2. 영상탭 수정
- **자막 근본 수정(CUT14)**: TTS탭 "합치기"가 subtitles[cutId]에 대사 전체 문자열을 덮어써 필드게이트 세그별 자막(CPP)을 가리던 문제 → TTS는 CPP 있는 컷엔 안 씀, 영상탭은 CPP 있는 컷의 문자열 오버라이드 무시, 일부 슬롯만 채운 컷에서 자막 수정해도 나머지 세그 자막 보존.
- **클립 파일명 표준화**: 슬롯 없는 업로드(`new<타임스탬프>`)는 저장 후 화면 순서 기준 `cut_NN_clip_<번호>`로 자동 정리 + 상단 "🏷 클립 파일명 정리" 버튼(`POST /api/rename-clip-files`, raw 폴더·cut_NN_clip_* 한정, 덮어쓰기 차단·삭제 없음).
- **유실 클립 복원**: 컷 9/11/12/13/18/19의 raw 사본 11개가 사라져 "재생할 수 없습니다" → 사용자 원본 폴더에서 stagedPath로 복원(사본이 사라진 원인은 미확인).
- 수동 조정 보호(override) 가드: UPDATE_CUT이 duration/segments/narration/dialogue/imagePrompt/videoPrompt/segPrompts/directionNote 수정 시 자동 기록, 서버 저장 시 오래된 탭/자동화의 되돌리기 차단(전 세션 작업, 이번 세션에서 실동작 확인).
- 컷카드 "🔍 AI 진단" 패널 신규(`DiagnosisPanel.jsx`, Field Gate 진단 챗 이식, 반영은 UPDATE_CUT + SEG 8/10 가드) — 로드맵 ① 완료, 반영 버튼 실사용은 미검증.

### 3. 메이킹탭 효과음
- 릴스 최종본 표 SFX열에서 컷별 효과음 직접 지정(masterCode.audio.sfxFile/sfxAt, 서버 decideCut이 키워드 규칙보다 우선, `__none__`=없음), AI 생성 효과음이 SfxPicker 카탈로그 "🆕 AI 생성" 카테고리로 노출(대본생성·편집메타 탭 포함).

### 4. 유료 영상 API 준비 + 크레딧/비용
- 서버 유료 사용 장부(`server/lib/paidUsage.js`, `/api/paid-usage/*`: 요약·기록·환율·예산 검사) — 별도 파일이라 탭 저장이 덮어쓰지 못함. 대시보드 "이번 달 비용"이 직접 입력 + 자동기록 합산, 예산 초과 경고.
- 크레딧 탭: 배정처 메인/부/유료 API/내일로 미룸 + "🧮 무료 우선 자동 배정"(G4 미승인 서여리 컷을 클립 단위로, 이미 채워진 슬롯 제외) + 유료 API 사용 카드(수동 기록·환율). 공급자 미정 — 연결부는 교체형으로 설계, Veo API 공식가(2026-09-20): Lite 720p $0.05/s · Fast $0.10~0.12/s · Standard $0.40/s(무료 구간 없음, 구독과 API 결제는 별개), 이번 에피소드 서여리 컷 약 256초 ≈ Fast 720p 1회 생성 약 $26.
- 로드맵(Field Gate 통합): ①진단 패널 완료, ②세그 빌더 영상탭 통합·③Veo/유료 API 자동생성·④Field Gate 축소는 남음.

### 남은 것
Flow 버튼 실사용 통합 검증(다음 에피소드), 유료 API 연결(계정 결정 후), 실패 자동 재시도, PixVerse 자동화, 진단 패널 반영 버튼 실사용 검증, CheckupTab은 세그별 자막 미사용(별개 로직).

커밋: 이번 세션 변경은 hourly auto-sync(매시 36분)가 커밋 — 개별 해시 미기록.


---
### 2026-09-20 (MCP 자동 기록)
## [사후 보완] 2026-09-13 ~ 09-19 작업 요약 (STATUS 공백 구간 재구성)

※ 이 절은 9/21에 메모리 노트·git 기록·소스 주석을 근거로 **사후 재구성**한 것. 날짜별 세부는 원 세션 기록이 아니라 근거 기준이라 일부 누락·일자 오차 가능. (직전 절 "2026-09-12 Field Gate 세그 빌더" 이후, "2026-09-20~21 Flow 반자동화" 이전)

### 09-13 — CapCut 커터 재구축 + 체크업 탭 고도화
- `run-cutter.js`(CapCut draft_content.json 직접 기입 방식) 6개 기능 실측: CapCut 실행·새 프로젝트 복제(템플릿)·컷별 개별 클립 배열·켄번스는 이미 자동화, **대사 앞/뒤 패딩·BGM 배열은 없음**(BGM은 템플릿 audio 트랙 확장 가능). 설계 원칙: "배치까지 자동, BGM·색보정·내보내기는 사람".
- 체크업 탭(`CheckupTab.jsx`) Tier 3~5 구현·실측: 타임라인 편집기(분할/트림/드래그 재배치/삭제/실행취소·재실행), CapCut 스타일 툴바, 실시간 눈금자+연속 줌, 오디오 파형 라인, 1초 단위 스틸컷 필름스트립, 목업 기준 레이아웃(플레이어 우측 고정+좌측 "추후 반영"+통합 사이드바). 사용자: "몇 차례만 더 고도화하면 캡컷 못지않은 도구".
- 장기 방향 확정: 스튜디오가 캡컷급 편집기까지 흡수(AI 생성만 외부 API). 백로그 3건 지목: 사이드바 라이브러리(에셋), 효과연출(GRAPHIC_MOTIONS 재사용), **G5 생성파일 합성(체크업 탭이 최종 영상 생성)**. 미수정 잔존: SRT 폴백의 크로스에피소드 오염 버그.

### 09-15 — 로드맵 확정 + 대본/프롬프트 QC
- 사용자 로드맵: 며칠간 스튜디오 점검·필드게이트·트렌드레이더·에이전트 연동 보완 → **~9/22 인스타 피드·릴스 업로드 가능, ~9/29 현 단계 자동화 완료 + 유튜브 숏폼·롱폼 준비**.
- CUT2 "맨발 버전" 이미지에서 배경 인물·얼굴 drift 발생 → 근본 원인 조사: 룰셋에 "실내는 신발 벗는다" 지시가 없어 영문 프롬프트에 sneakers 등이 명시돼 있었음. **`applyIndoorBarefootGuard()`** 신설(ScriptGenTab.jsx + scriptParserV3.js 양쪽): 실내 거주공간 키워드+신발 연출 신호 없음이면 쉼표 절 단위로 신발 절만 제거하고 "Barefoot or socks only" 추가(첫 버전이 줄째 지워 의상 묘사가 날아가는 버그 → 절 단위로 재작성). StudioTab TOOL_SUFFIX 죽은 코드 발견, CUT1/2 이미지 뒤섞임 반복 버그와 md5 비교법 기록.

### 09-16 — ElevenLabs 부가기능·드래그 배정·Field Gate 안정화
- **ElevenLabs Starter 부가기능 4종**: Sound Effects(생성+`_shared/sfx/_generated` 라이브러리 저장, 실API 검증), Instant Voice Cloning(TTS탭 캐릭터별 "이 목소리로 클론"), Voice Isolator·Speech-to-Text(음성삽입 탭 "노이즈 제거"/"스크립트 추출", 권한 수정 후 실동작 검증). Pronunciation Dictionary만 보류. 서버는 `forwardMultipartToElevenLabs()` 공용 헬퍼.
- **스튜디오 이미지 "드래그 배정"**(`POST /api/upload-cut-image`): Flow 다운로드 파일명이 해시라 자동 컷 추정 위험 → 사람이 썸네일 보고 배정. **A/B 슬롯 2개 제약**(scan-media 정규식이 `_c` 이상을 새로고침 시 지움). 영상 탭 드래그 배정에서 stale-closure로 배정 클립이 통째로 사라지는 치명 버그 수정(UPDATE_TAB_FIELD 리듀서 방식). TTSTab에 동일 버그 미수정 잔존.
- **Field Gate 안정화**: studio-state를 boot 때 한 번만 캐시해 옛 프롬프트를 보여주던 문제 → 컷 전환마다 자동 새로고침, SEG 캐시(segState) 초기화. studio-state.json이 컷 배열을 `cuts`와 `episodes[활성].cuts` 두 곳에 중복 보관해 직접 patch가 한쪽만 반영되던 근본 원인 발견 → 저장 경로 2곳에 동기화 안전망. "IP 프롬프트에 반영" 버튼이 실제로는 복사만 하던 결함 → `POST /api/genline/apply-ip-prompt`로 즉시 반영.
- 커밋 `ab60ffd`: 카카오 5초 타임아웃 비동기 응답 처리.

### 09-17 — 동기화 중단·화면녹화·PIP·에이전트 리더
- **OneDrive 양방향 동기화 중단**: `sync-content.bat`의 robocopy가 `/E /XO`뿐(삭제 미전파)이라 로컬에서 지운 파일이 다음 사이클에 OneDrive 사본으로 되살아나 "고쳐졌다 며칠 뒤 재발"하던 버그(CUT1/2 이미지 뒤섞임 등)의 진짜 원인. 회사 PC를 더 안 써서 `start_yeori.bat`의 호출을 주석 처리(1대 PC 체제).
- **화면 녹화 무음 버그 수정**: `screen-recorder.js`가 오디오 입력이 없어 모든 BROLL/CapCut 화면녹화가 무음이었음 → 이 PC의 "스테레오 믹스" dshow 장치로 시스템 오디오 캡처(없으면 영상만 폴백), `hasAudio` 표시, 실녹화 ffprobe 검증. 신규: 윈도우 녹화 구조, 검색메타+브라우저 열기 UI.
- **PIP 합성 신규**(`cut.pipSegments`): 서여리 리액션 세그를 뮤비 컷(BROLL) 위 우하단 약 30% 창으로 합성, 서여리 대사 풀볼륨+뮤비 원음 18%(BGM처럼), CUT3/5/7 적용. 설계만 있던 pipTarget/pipLayout/pipScale에 실제 합성 코드 구현. 엔드투엔드는 synthetic만 검증.
- 에이전트 리더 백본: pipeline-leader/genline이 Notion을 결정 원장으로 사용 — P1(리더 로그 DB)·P2(에피소드 파이프라인 DB 매 사이클 동기화) 완료, 다음 P3(읽기·정책, `shouldAutoApprove` 배선). (정확한 구현 일자는 9/9 설계~9/17 사이)
- 사용자 요청: 웬만한 확인은 생략하고 되돌릴 수 있는 작업은 바로 실행.

### 09-18 — PIP 재설계 + 영상탭 정비 (소스 주석 기준)
- **PIP 재설계**: 리액션은 컷3/5/7(발화 컷)의 클립을 그대로 재사용(중복 업로드 제거), PIP 대사가 화면보다 먼저 들리던 시작 타이밍 어긋남 수정.
- 영상탭: 컷카드 영상 박스가 옆 패널 높이만큼 늘어나 레터박스가 생기면서 자막 오버레이가 화면 밖처럼 밀리던 진짜 원인(`.cutCardRow` stretch) → 박스를 영상 비율로 고정. 자막 줄바꿈 반영, 자막 편집창이 재생 버튼과 겹치던 위치 문제, "⚠ 서버 미반영" 오경고(stagedPath 미기록), 스트림 파싱 "처리중" 멈춤, 컷17 파일 없을 때 404 처리, 썸네일 누락, 최종 합성본 미리보기의 자막 라이브 오버레이 제거(사용자 확정).
- 컷20 믹스: BGM이 나레이션으로 대체돼 음악이 안 들리던 문제 수정, 세 트랙(배경/나레이션/효과음) 시작시간·음량 UI를 한 줄에 정리.

### 09-19 — 제작 작업일 (소스 변경 없음)
- git 기록상 코드 변경 없이 상태·미디어 파일만 갱신: `05_video`의 컷 파일(컷2~9, 11~13, 18, 19; 14:30~19:00)과 `studio-state.json`·`gpoints.json`. 클립 배정·합성 등 사용자 제작 작업일로 보임.

### 이 구간에서 남은 미해결
TTSTab stale-closure 클립 소실(미수정), SRT 폴백 크로스에피소드 오염, PIP 자막 좌측정렬 미구현, PIP 엔드투엔드 미검증, 에이전트 리더 P3~P4, Pronunciation Dictionary 보류.


---
### 2026-09-20 (MCP 자동 기록)
## Notion 마스터 허브 정리 + 토큰 0 자동 갱신 규칙 구축 (2026-09-21)

- **허브 정리**: 마지막 편집이 9/7이라 컷 번호 체계(1~12/91·92/101·102/201~206 → 9/8 순차 번호로 폐기)·G2/G4 자동화 현황·동기화(OneDrive 중단)·Notion 갱신 담당 등이 현재와 어긋나 있었음. 최상단을 개정(확정 규칙 = 컷 1번부터 순차·`renumber_cuts`, 자동화 현황 표, 구조·환경·핵심 원칙, 업데이트 규칙)하고, 7~8월 세션 기록 381블록(하위 포함 401블록)은 삭제하지 않고 맨 끝 "📁 보관" 접기 블록으로 이동. 이동은 백업(`downloads/state/_hub-backup-20260921.json`) → 복사 → 재귀 검증(블록 수·종류별 수·글자 수 일치) → 원본 삭제 순서. 하위 DB 4개와 STATUS 미러 페이지는 건드리지 않음.
- **업데이트 규칙(토큰 0)**: 허브 맨 위 "🔄 자동 갱신 — 현재 상태" 블록을 코드만으로 갱신(`server/lib/hubSnapshot.js`, LLM 호출 없음). 원천은 로컬 파일(studio-state.json·gpoints.json·paid-usage.json·STATUS.md), 내용이 직전과 같으면 Notion에 쓰지 않음. 호출 지점은 기존 훅 재사용 — `syncLatestStatusToNotion()` 맨 앞(= `update_status_md` MCP 실시간 + 시간별 git-auto-sync의 `mirror-status-notion.js`). 수동 실행/규칙 검토일 기록: `node scripts/hub-refresh.js [--mark-reviewed|--print|--set-block=<ID>]`.
- **자동 경고**: STATUS.md 최신 절이 3일 이상 묵으면, 허브 "확정 규칙" 검토가 14일 이상 묵으면 상태 블록에 ⚠️ 표시(9/12~9/20 STATUS 공백 같은 일 재발 방지).
- 역할 분담 원칙: 반복 갱신=코드(토큰 0) / 사람·Claude=판단이 필요한 요약(`update_status_md` 1회)과 규칙 변경 시 "확정 규칙" 수정 + `--mark-reviewed`. 새 자동 갱신은 LLM 호출 금지.
- 잔여: 허브 "환경 정보"/"자동화 현황"은 수동 갱신 섹션(구조가 바뀔 때만), Notion 페이지 육안 확인은 사용자 몫.


---
### 2026-09-20 (MCP 자동 기록)
## 승인 게이트 정책 정리 + 결정론적 검수기 (2026-09-21)

- **배경**: G4 승인이 1/23 인데 영상은 이미 전부 올라가 있었음(수동 업로드·드래그 배정은 승인을 안 찍음, G4 승인은 G5 자동 실행의 조건). 기존 리더 자동승인은 메이킹 컷 G4 한 가지(mp4 존재·매니페스트·미반려)뿐이고 G2/G3/G4(서여리)는 검수 로직 없음 → 항상 사람 대기였음.
- **정책 문서**: `app/docs/gate-approval-policy.md` — 승인 정의(사람의 확정 신호, 파일 존재와 별개), 판정 5단계(approved/auto_ok/recommend/blocked/not_ready), 게이트별 하드·소프트·수동 검사표, 리더 위임 규칙, 사람 승인 절차, 후속(P4). 허브에도 요약 섹션 추가.
- **검수기(토큰 0)**: `server/lib/gatePolicy.js` + `scripts/gate-eval.js`(CLI) + `GET /api/gate-eligibility`·`GET /api/mcp/gate-eligibility`(자식 프로세스 실행, 서버 비차단, 디스크 캐시 `.gate-cache.json`로 재검수 2초). 기준: G3 = 파일 정상·길이≤DU+0.5초·무음 아님·클리핑·대본 최신성(위임 가능) / G4 = 비디오 스트림·화면 비율·길이 DU±1초·세그 슬롯 충족·반려 없음·대사 컷 오디오·검정 화면(메이킹 컷만 위임, 서여리 컷은 립싱크·연기가 시각 판단이라 사람 1클릭) / G1·G2·G5 사람 전용.
- **리더 연결(`pipeline-leader.js`)**: 메이킹 G4 자동승인이 검수 `auto_ok`까지 요구(fail-closed), G3 위임 시 검수 통과 컷만 자동 승인하는 호출부 추가, blocked 컷은 리더 로그 블로커로 기록. 위임 스위치(Notion "에이전트 자동승인", 기본 비어 있음)가 꺼져 있으면 기존과 동일하게 아무것도 승인하지 않음. 리더 재시작 시 반영.
- **LF_T01 검수 결과**: G4 승인 1 · 권고(recommend) 13 · 위임 가능(auto_ok) 4(BROLL) · **막힘 3** · 산출물 없음 2. 막힘 = 컷14(최종 영상이 9/9 옛 파일 10초·오디오 없음 → 3클립 24초 합성 필요), 컷10·15(GRAPHIC 영상 길이가 DU와 불일치). 산출물 없음 = 컷16·21(클립은 채워졌으나 합성본 없음). G3 위임 가능 2. 허브 상태 블록에 "승인 현황" 줄 추가.
- **승인 자체는 하지 않음**(사용자 판단 몫): 일괄 승인 여부·시점은 사용자가 결정.
- 잔여: 영상 탭 "검수 통과 컷 일괄 승인" 버튼, 승인 주체 기록(approvedBy), 프레임 QC·STT 립싱크 점검(P4), G5 검수.


---
### 2026-09-21 (MCP 자동 기록)
## 2026-09-21 이미지·영상 자동화 구축 + 음성 검수 + IG_R04 제작

### IG 시작 준비 (첫 업로드 9/25 금)
- IG_R02 자막(CP) 연결·제목 확정("AI인지 모르고 DM 보낸 사람들, 이거 실화?"). IG_R04(서여리 채널소개 1화_저 사실... ㅇㅇ로 만들어요.) 대본 확정(3컷 8/10/8초, A안 대사, 나레이션은 자막)·스튜디오 에피소드 연결. 기획 자료 2개 통합(instagram-launch-plan.md v0.3, AI 표시 의무 반영).
- R04 컷1: 기존 영상 재사용(자막은 3.5초 이후 나레이션 — `captionStartSec` 신설, 미리보기·최종본 렌더 반영). 이미지 확정: 컷1 cut_01_b / 컷2 cut_02_d / 컷3 cut_03_b(G2 승인 기록). 영상: 컷2 Flow 반자동 성공(cut_02_clip_1.mp4), 컷3은 사용자가 Flow 에이전트로 제작(cut_03_clip_1.mp4).

### Flow 이미지 자동 생성(신규)
- `scripts/flow-image.js` + flowDriver 이미지 모드(Nano Banana 2, 9:16, x2, 레퍼런스 첨부 — 얼굴 클로즈업+추가 의상 레퍼런스 extraRefs, 한 줄 입력·일치 검증, 하루 180장 카운터·컷 간 20초 간격, 다음 빈 슬롯 저장). `POST /api/flow/image`, `GET /api/flow/image-usage`. 이미지 생성은 0 크레딧(하루 한도 내 무료). 이미지용 Flow 프로젝트는 downloads/state/flow-image-project.json.
- 화면 언어는 `?hl=` 로 고정(이미지 en / 영상 ko). 참조 썸네일은 프롬프트 창 안만 센다(툴팁 오검출 수정).

### Flow 영상 제출 개선
- 비율 자동(IG=9:16)·x1 고정·영상 모드 전환·소재 모드(참조 2장)·종료 프레임·시작 프레임/프롬프트 직접 지정. 컷2 성공(재시도 1회), 컷3은 자체 제출 6회 실패.
- **Flow 경고 발견**: 실패 타일에 "비정상적인 활동이 감지되었습니다" 표시 → 감지 시 자동 제출 중단·잠금(`flow-video-blocked.json`) 로직 추가(사용자 확인 후 잠금 해제함). 반복 자동 제출은 계정 제한 위험 → 제출 간격 확대·하루 횟수 제한 설계 필요.

### 에피소드별 탭 상태 분리(중대 버그 수정)
- videoTabState·ttsTabState·voiceInsertState·studioTabState 가 컷 id("cut-1")로 앱 전체 공유 → 다른 에피소드 컷1 업로드가 모든 에피소드에 덮어써지던 문제. 활성 에피소드만 최상위, 나머지는 episodes[id].tabState 로 전환 시 맞바꿈(클라이언트+서버). 기존 저장본은 stagedPath 로 소유자 분리, 기본값 공유 객체 오염(내 마이그레이션 실수) 수정·정리. 관련 화면: 9:16 미리보기 2/3 크기, 컷목록 세로 썸네일(thumbAspect), 자막 시작 지연 미리보기.

### 음성 연구·검수기
- 실측: STS 는 발음 오류를 못 고침(입력 억양·발음 추종). STS 설정 차이는 실행 간 변동보다 작음. Veo 원본끼리 음색 유사도 0.79~0.83 = 귀로 "거의 같음"(사용자 확인), Veo↔서여리 TTS(Sian) 0.6~0.75. → 발음 통과 컷은 원본 그대로, STS 는 예외.
- 서여리 TTS 탭 값 안정성 45·유사도 75(속도 0.9 유지; STS 속도 1.0 고정)로 변경, 기존 값은 voice_presets.json 에 보관. 한지아는 현행(20/70) 유지.
- **음성 검수기**: `server/lib/voiceQa.js`·`scripts/voice-qa.js`(STT 대본 비교·신뢰도·애드립), `voice_embed.py`(음색 일관성, 기준 0.75), 영상 저장 직후 자동 실행, `GET /api/voice-qa`·`POST /api/voice-qa/run`, 게이트 G4 `voice-qa` soft 검사, 영상탭 🎙 배지·"음성 검수" 버튼. 오류 사례 2건(중심으로→중십프로 = warn, 한지아 애드립 = fail) 검증.
- **말투(register)**: viewer(시청자·나레이션, 차분) / friend(친한 친구 대화, 텐션↑). 컷 `register` 수동 지정(영상탭 🎭 선택) 우선, 미지정 시 CH 에 HJ 있으면 friend. 영상 프롬프트 "Vocal delivery" 문장·STS 설정에 반영.

### 남은 일 / 주의
- 음성 검수 결과는 다음 진행에서 사용자 확인 예정. 게이트 화면에서 음성 검수 경고 실표시 확인. TTS 생성은 아직 말투 미구분. friend 수치 귀 확인. 한지아 별도 결.
- Flow 영상 자동 제출: 간격·일일 횟수 제한 설계 후 재개. 스튜디오 이미지 탭 "Flow로 생성" 버튼 미구현. 컷1 영상(6/23 기존)이 컷2·3과 조명·의상 이질 시 cut_01_b 로 재생성 검토.
- 파이프라인 리더 위임 정책은 서여리 컷 G4 사람 1클릭 유지. R02 컷4·R03 제목/번호/컷3 길이 등 이전 미결 항목 유지.


---
### 2026-09-22 (MCP 자동 기록)
## R03·R04 릴스 최종본 정리 마무리 (2026-09-22)

**R04(채널소개 1화)**
- 자막 순차분리 버그 수정(구간별 "/" 안 나뉘던 것), 컷1·3 나레이션/대사 TTS 추가·립싱크 타이밍 동기화
- 컷3 정지→후반부 립싱크 반전 연출 확인 후 나레이션은 생략, 속마음 자막만 복원
- 컷1 누락됐던 대사 자막("어, 왔어요?") 복원, 영상탭 미리보기 재생위치 버그 수정
- SFX/자막을 studio-state.json과 완전히 분리된 오버라이드 파일로 저장하는 구조 신설 → 브라우저 자동저장이 되돌리는 사고 원천 차단
- 최종본 스테일 자동감지, 영상/오디오 길이 드리프트 자동보정 파이프라인에 반영
- IG_R 콘텐츠가 실제 대사영상도 "이미지+모션"으로 오분류되던 문제 수정(videoPolicy 지정)

**R03(AI 크리에이터 비용 0원)**
- 컷5가 실은 정지 일러스트가 아니라 후반부(6.1~7.6초)에 립싱크하는 반전 연출이었음을 재확인, 원본 소스로 교체·재동기화
- 컷4·5 영상탭 클립 참조가 옛 원본 파일을 가리켜 재생 안 되던 문제 수정
- 컷4·5 videoMode 오분류 수정

**다음 일정**: 9/25 첫 업로드 예정 티저 시리즈 제작 시작(내일).
