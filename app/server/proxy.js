...
mcpRouter.post('/update-status-md', async (req, res) => {
  ...
  return {
    ...
    preSteps: [
      ...
      { action: 'eval', name: 'fullbleed', optional: true,
        fn: "() => { const hide='ytd-masthead,#masthead-container,tp-yt-app-header,#secondary,#below,ytd-watch-metadata,#related,.ytp-chrome-top,.ytp-chrome-bottom,.ytp-gradient-top,.ytp-gradient-bottom,.ytp-ce-element,.ytp-pause-overlay,.ytp-popup,.iv-branding,.ytp-caption-window-container,.captions-text,tp-yt-iron-overlay-backdrop'; document.querySelectorAll(hide).forEach(e=>e.style.setProperty('display','none','important')); const cc=document.querySelector('.ytp-subtitles-button[aria-pressed=\"true\"]'); if(cc)cc.click(); const v=document.querySelector('video'); if(v){v.style.cssText='position:fixed!important;top:0!important;left:0!important;width:100vw!important;height:100vh!important;object-fit:cover!important;z-index:2147483647!important;background:#000'} const st=document.createElement('style'); st.textContent='html,body{overflow:hidden!important;margin:0!important}#page-manager,ytd-app{margin:0!important;padding:0!important}'; document.head.appendChild(st); if(v&&v.paused)v.play&&v.play().catch(()=>{}); }" },
    ],
    steps: [
