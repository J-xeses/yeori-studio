...
// 사용: node scripts/render-teaser.mjs <html> <outDir> [--v=black,gold,...] [--dur=6.5] [--stills=0.5,2,3.5]
...
const variants = (opt.v || 'black').split(',')
...
try {
  for (const v of variants) {
    ...
    const url = pathToFileURL(path.resolve(htmlPath)).href + (v === 'black' ? '' : `?v=${v}`)
