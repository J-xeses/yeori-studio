...
|배경 인물|공용 공간(카페·거리 등)은 배경 인물 허용 / 단 서여리 행동·연출에 개입 금지 → "background people must not interact with or interfere with the main character" 필수. **단, 서여리 집 소파 등 "지정 인물만 있어야 하는 사적 공간"은 배경 인물 자체를 금지** — "no other people in background, only the named characters in frame"(2026-09-15, 실측: 집 소파 컷에 사적 공간인데도 배경 인물이 생성돼 지적됨. 코드 레벨 자동 치환 장치도 있음 — [[project_script_prompt_qc_guards]] 참고)|
|얼굴 클로즈업|일관성 가장 높음 → 적극 활용|
|실내 신발|실내 캐주얼 장면은 기본 맨발/양말 — 신발 착용은 ④ "하이힐=페르소나" 같은 의도된 상징 연출일 때만. 명시 지시 없으면 항상 신발 벗은 상태로 프롬프트 작성(2026-09-13, 실측: 소파 캐주얼 컷에 스니커즈 신은 채 생성돼 지적됨)|
|화면 내 텍스트|생성 영상 안에 자막/UI/깨진 글자가 보이면 무조건 결함(모델이 "NO subtitles" 지시를 무시하고 그려낸 것) — 즉시 재생성. 폰/TV/노트북 화면이 프레임에 들어가면 화면 내용은 흐릿하게/추상적으로만 묘사, 화면에 읽을 수 있는 텍스트가 나온다고 서술하지 않기(2026-09-13, 실측: 폰 보여주는 컷에 깨진 한글 자막이 화면에 나타남)|
|화면(TV 등) 노출 연출|뮤비/영상 화면 자체를 보여줘야 하는 컷은 인물을 화면 앞에 **뒷모습**으로 배치. 뒷모습(화면 보임)→정면 MCU 반응(화면 안 보임) 전환은 카메라가 캐릭터를 물리적으로 돌아야 해서 한 연속 샷으로 잇기 어려우므로, 단일 videoPrompt의 시간 구간이 아니라 **SEG로 컷을 분할**(예: SEG 4+6)해 세그별 독립 클립으로 하드컷할 것(2026-09-15, 사용자 제안, CUT11에 실측 적용)|
|레퍼런스 이미지|REF(face/closeup) 이미지는 캐릭터 일관성 참고용 "입력"일 뿐, 그 자체가 컷의 최종 영상/이미지 결과물로 저장·사용되면 안 됨 — 결과물의 배경 인물·의상이 그 컷의 실제 설정과 다르면 레퍼런스가 그대로 흡수된 것이니 즉시 재생성(2026-09-13, 실측: 앞 컷과 다른 배경 인물이 섞인 레퍼런스 클로즈업이 최종 영상에 그대로 들어감)|
...
□ 실내 캐주얼 장면은 맨발/양말(의도된 신발 연출이 아니라면)
□ 사적 공간(집 등)은 배경 인물 자체가 없음(지정 인물만)
□ 화면 내 자막/UI 텍스트 없음, 폰·TV 화면은 흐릿하게만 묘사
□ 화면(TV 등) 노출이 필요한 컷은 뒷모습(SEG1)+정면 MCU(SEG2) 분할 검토
□ 결과물이 레퍼런스 이미지 그대로가 아님(배경·의상이 그 컷 설정과 일치)
```
