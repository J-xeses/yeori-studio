...
[자막(CP) 설계 — 컷마다 필수 검토, "없음"을 기본값으로 방치 금지]
자막(CP 필드)은 부가 요소가 아니라 대사·나레이션과 동급의 핵심 연출 요소다. 모든 컷에서
"이 컷에 화면 자막이 필요한가"를 명시적으로 판단하고, 필요하면 CP 필드에 짧은 문구(순수
텍스트, 3~8단어 내외)를 반드시 채운다.
- BROLL — 서여리 얼굴이 없어 대사가 없는 컷이 대부분이라, 화면만으로는 맥락 전달이 약하다.
  "데뷔 N년차 / 핵심 키워드" 식의 짧은 정보 자막(CP)을 기본으로 넣는다. 나레이션이 그 장면을
  설명하는 컷은 예외적으로 CP를 비우고(NR만 사용), 그 경우 액션 필드에 "자막 없음(나레이션이
  설명)" 이유를 남긴다.
- GRAPHIC — 자막/텍스트 카드가 컷의 본체이므로 CP는 사실상 필수.
- CAPCUT — "자막만 있는 컷"이 정의이므로 CP 필수.
- YEORI/PIP — 대사가 이미 화면 자막(립싱크 대사) 역할을 하므로 보통 CP는 비워도 되지만,
  강조하고 싶은 핵심 문장이 있으면 CP에 짧게 반복해도 된다.
- CP를 비울 때는 반드시 의도적 판단 결과여야 한다("자막 없음"을 그냥 기본값으로 두지 말 것).
...
□ 이 컷의 CP(자막) 필요 여부를 판단했음 — BROLL/GRAPHIC/CAPCUT은 기본적으로 CP 채움,
