        ...
        {channelSaved && (
          <div className={s.section}>
            ...
            <button className={`${s.genBtn} ${canGenerate ? s.genBtnActive : ''} ${s.mtop}`}
              ...
              {loading
                ? <span className={s.loadingDots}>기획 생성 중<span>.</span><span>.</span><span>.</span></span>
                : '🎬 시리즈 기획 개요 생성'}
            </button>
            {error && <div className={s.error}>{error}</div>}
          </div>
...
export default function StoryArchiveTab() {
  const [activeInner, setActiveInner] = useState('short')
    ...
    <div className={s.wrap}>
      <div className={s.header}>
        ...
        <div>
          ...
          <p className={s.subtitle}>트렌드 × 서여리 = 콘텐츠 시너지</p>
        </div>
      </div>

      <div className={s.innerTabs}>
        <button className={`${s.innerTab} ${activeInner === 'short' ? s.innerTabActive : ''}`}
          onClick={() => setActiveInner('short')}>
          💡 단편 아이디어
        </button>
        <button className={`${s.innerTab} ${activeInner === 'series' ? s.innerTabActive : ''}`}
          onClick={() => setActiveInner('series')}>
          🎬 시리즈 기획
        </button>
      </div>

      {activeInner === 'short' ? <ShortIdeaTab /> : <SeriesPlanTab />}
    </div>
  )
}
