...
> ⚠️ **v1.3 안내**: 샷 타입 접두어 표기(`CLOSEUP SHOT —` / `FULLBODY SHOT —`)는 ⑬의 `SH_` 코드 체계로 대체되었다. 이 섹션의 분류 기준(용도·이미지 생성 방식)은 유효하나, 실제 프롬프트/필드에 쓰는 표기는 ⑬을 따를 것.
